...
class TransformCsvGuildUpgradeDescriptionTranslation(transform_csv.TransformCsvTask):
        ...
        return [
            {
                ...
                "translation": common.to_xhmtl_fragment(guild_upgrade_description),
            }
        ]

    def requires(self):
        return guild_upgrade_extract.ExtractBatch(lang_tag=self.translation_lang_tag)
...
class TransformCsvGuildUpgradeNameTranslation(transform_csv.TransformCsvTask):
    app_name = luigi.Parameter(default="gw2")
    original_lang_tag = luigi.EnumParameter(enum=common.LangTag)
    task_datetime = luigi.DateSecondParameter(default=config.gconfig().task_datetime)
    task_namespace = "guild_upgrade"
        ...
        return [
            {
                ...
                "original_lang_tag": self.original_lang_tag.value,
                "translation_lang_tag": self.translation_lang_tag.value,
                "translation": common.to_xhmtl_fragment(guild_upgrade["name"]),
            }
        ]
