...
CREATE TABLE gwapese.novelty_description (
  ...
  CONSTRAINT novelty_description_pk PRIMARY KEY (app_name, lang_tag, original,
    novelty_id),
  CONSTRAINT novelty_description_u UNIQUE (app_name, lang_tag, novelty_id),
...
CREATE TABLE gwapese.novelty_description_context (
  ...
  CONSTRAINT novelty_description_context_pk PRIMARY KEY (app_name,
    original_lang_tag, translation_lang_tag, novelty_id),
  CONSTRAINT novelty_description_sources_context_fk FOREIGN KEY (app_name,
    original_lang_tag, original, novelty_id) REFERENCES
    gwapese.novelty_description (app_name, lang_tag, original, novelty_id) ON
...
CREATE TABLE gwapese.novelty_name (
  LIKE gwapese.copy_source,
  novelty_id integer NOT NULL,
  CONSTRAINT novelty_name_pk PRIMARY KEY (app_name, lang_tag, original, novelty_id),
  CONSTRAINT novelty_name_u UNIQUE (app_name, lang_tag, novelty_id),
...
CREATE TABLE gwapese.novelty_name_context (
  ...
  CONSTRAINT novelty_name_context_pk PRIMARY KEY (app_name, original_lang_tag,
    translation_lang_tag, novelty_id),
  CONSTRAINT novelty_name_sources_context_fk FOREIGN KEY (app_name,
    original_lang_tag, original, novelty_id) REFERENCES gwapese.novelty_name
    (app_name, lang_tag, original, novelty_id) ON DELETE CASCADE ON UPDATE
    CASCADE,
